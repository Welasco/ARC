...
if (-not $cert) {
    AddLog "Certificate $keyVaultCertificateName not found in Key Vault $keyVaultName. Attempting to import certificate from local files."
    $localCertFilesPath = @("$machineName.pem", "$machineName.pfx")
    foreach ($certFilePath in $localCertFilesPath) {
        if (Test-Path $certFilePath) {
            AddLog "Importing certificate $certFilePath to Key Vault $keyVaultName as $keyVaultCertificateName"
            $cert = Import-AzKeyVaultCertificate -VaultName $keyVaultName -Name $keyVaultCertificateName -FilePath $certFilePath
            break
        }
        else {
            AddLog "Certificate file $certFilePath not found."
        }
    }
    if (-not $cert) {
        AddLog "Certificate $keyVaultCertificateName not found in Key Vault $keyVaultName and attempting to import from local files not found. Exiting."
        exit 1
    }
}
